        ...
        if layer is not None:
            ...
            for group in layer["groups"]:
                if group["name"] == PRIORITY_GROUP.CARBON_IMPORTANCE.value:
                    ...
                    self.climate_resilience_box.setValue(int(group["value"]))
                elif group["name"] == PRIORITY_GROUP.ECOLOGICAL_INFRASTRUCTURE.value:
                    self.ecological_box.setValue(int(group["value"]))
                elif group["name"] == PRIORITY_GROUP.POLICY.value:
                    self.policy_box.setValue(int(group["value"]))
                elif group["name"] == PRIORITY_GROUP.FINANCE_YEARS_EXPERIENCE.value:
                    self.finance_experience_box.setValue(int(group["value"]))
                elif group["name"] == PRIORITY_GROUP.FINANCE_MARKET_TRENDS.value:
                    self.finance_market_box.setValue(int(group["value"]))
                elif group["name"] == PRIORITY_GROUP.FINANCE_NET_PRESENT_VALUE.value:
                    self.finance_present_value_box.setValue(int(group["value"]))
                elif group["name"] == PRIORITY_GROUP.FINANCE_CARBON.value:
                    self.finance_carbon_box.setValue(int(group["value"]))
